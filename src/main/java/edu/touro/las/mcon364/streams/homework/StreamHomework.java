...
public class StreamHomework {
    ...
    public double getTotalRevenue() {
        ...
        return Optional.of(customerOrders.stream().filter(x -> x.status == OrderStatus.DELIVERED)
    ...
    public double getAverageOrderValue() {
        ...
        return Optional.of(customerOrders.stream()
                .filter(x -> x.status == OrderStatus.DELIVERED)
                .mapToDouble(CustomerOrder::getTotal)
                .average().orElse(0.0)).get();
    }
    ...
    public Map<String, Double> getRevenueByCustomer() {
        ...
        return customerOrders.stream()
                .filter(x -> x.status == OrderStatus.DELIVERED)
                .collect(Collectors.groupingBy(y -> y.customerId,
    ...
    public List<String> getTopCustomers(int n) {
        ...
        return getRevenueByCustomer().keySet().stream().sorted(Comparator
                        .comparingDouble(x -> getRevenueByCustomer().get(x)).reversed())
                .limit(n)
                .toList();
    }
    ...
    public List<String> getCustomersWithMultipleOrders() {
        // TODO: Implement using streams
        return getCustomerOrderCounts().keySet().stream()
                .filter(x -> getCustomerOrderCounts().get(x) > 1).toList();
    }
    ...
    public Map<String, Double> getRevenueByCategory() {
        ...
        return customerOrders.stream()
                ...
                .collect(Collectors.groupingBy(y -> y.product().category(),
    ...
    public Map<String, Integer> getProductQuantitySold() {
        ...
        return customerOrders.stream().flatMap(x -> x.items().stream())
                .collect(Collectors.groupingBy(x -> x.product().id(),
                        Collectors.summingInt(x -> x.quantity)));
    }
    ...
    public List<CustomerOrder> getOrdersInDateRange(LocalDate start, LocalDate end) {
        ...
        return customerOrders.stream()
                .filter( x -> !x.orderDate().isBefore(start)
                && !x.orderDate().isAfter(end)).toList();
    }
